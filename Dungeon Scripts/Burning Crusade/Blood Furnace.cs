...
#endif
{
    public class BloodFurnace : Dungeon
    {
        ...
        public override uint DungeonId { get { return 137; } }
            ...
            return new PrioritySelector(ctx =>
                ...
                // clear the area before we start the event
                        ...
                        new Decorator(
                            ...
                            new PrioritySelector(
                                new Decorator(ctx => entranceDoor.State == WoWGameObjectState.Active,
                                    ScriptHelpers.CreateInteractWithObject(181982)),

                                new Decorator(ctx => entranceDoor.State == WoWGameObjectState.Ready,
                                    new Action(ctx => ScriptHelpers.MoveTankTo(tankPoint))))))));
        }
        ...
        {
            return new PrioritySelector(ctx => _kelidan = ctx as WoWUnit,
                ScriptHelpers.CreateRunAwayFromBad(ctx => _kelidan.HasAura("Burning Nova"), () => _kelidan.Location, 30, 20f, 17377));
        }
    }
}