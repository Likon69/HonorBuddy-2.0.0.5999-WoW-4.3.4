...
#endif
{
    public class HallsOfOriginationHeroic : Dungeon
    {
        #region Overrides of Dungeon

        public override uint DungeonId { get { return 321; } }
            ...
            units.RemoveAll(
                ret =>
                {
                    ...
                    if (ret.Entry == TempleGuardianAnhuur && ret.ToUnit().HasAura("Shield of Light") &&
                        !StyxWoW.Me.IsHealer && StyxWoW.Me != _anhuurInterrupter)
                        return true;
        ...
        public override void IncludeTargetsFilter(List<WoWObject> incomingunits, HashSet<WoWObject> outgoingunits)
        {
            foreach (var obj in incomingunits)
            {
                ...
                if (unit != null)
                {
                    if (unit.Entry == TempleGuardianAnhuur && unit.Combat &&
                        (StyxWoW.Me.IsHealer() || _anhuurInterrupter != null && _anhuurInterrupter.IsMe))
                        outgoingunits.Add(unit);

                    if (unit.Entry == PitViper && unit.Combat &&
                        ((!_anhuurIsCastingHymn && unit.X > 75 && unit.DistanceSqr < 40 * 40) || !_ignoreSnakes ||
                         (StyxWoW.Me.IsTank() && _anhuurIsImmune)))
                        outgoingunits.Add(unit);
        ...
        public override void WeighTargetsFilter(List<Targeting.TargetPriority> units)
        {
            foreach (var priority in units)
            {
                ...
                if (unit != null)
                {
                    if (unit.Entry == TempleGuardianAnhuur && unit.Combat && _anhuurIsCastingHymn &&
                        _anhuurInterrupter != null && _anhuurInterrupter.IsMe)
                        priority.Score += 500;

                    if (unit.Entry == PitViper && _anhuurIsImmune && StyxWoW.Me.IsTank() && unit.Combat &&
        ...
        private const uint SolorFire = 39634;
        ...
        private WoWPlayer _anhuurInterrupter;
        ...
        public Composite TempleGuardianAnhuurEncounter()
        {
            ...
            var hymnSpellIds = new[] { 75323, 75322 };
            ...
            return new PrioritySelector(
                ctx =>
                {
                    ...
                    _anhuurInterrupter = GetInterrupter();
                    partyMembers.Clear();
                    if (StyxWoW.Me.IsDps())
                    {
                        partyMembers.AddRange(
                            ScriptHelpers.PartyIncludingMe.Where(
                                p => p != healer && p != tank && p != _anhuurInterrupter && p.IsAlive)
                                .OrderBy(p => p.MaxHealth));

                        var maxHealthRank = partyMembers.IndexOf(StyxWoW.Me);
                        _anhuurBeaconSide = maxHealthRank == 0 ? AnhuurBeaconSide.Right : AnhuurBeaconSide.Left;
                    }
                    else if (StyxWoW.Me.IsTank())
                    {
                        _anhuurBeaconSide = AnhuurBeaconSide.Right;
                    }
                    else if (StyxWoW.Me.IsHealer())
                    {
                        _anhuurBeaconSide = AnhuurBeaconSide.Left;
                    }
                    ...
                    _anhuurIsCastingHymn = hymnSpellIds.Contains(boss.CastingSpellId);

                    var angrySnakes =
                        ObjectManager.GetObjectsOfType<WoWUnit>().Where(u => u.Entry == PitViper && u.Combat).ToList
                            ();
                    //_ignoreSnakes = angrySnakes.Count <= 2 && _anhuurIsImmune ||
                    //                !_anhuurIsImmune && _anhuurIsCastingHymn;
                ...
                // beacon behavior
                        new Decorator(
                            ctx => _anhuurIsImmune && _beaconOfLight != null,
                            new PrioritySelector(
                // handle interruption
                                new Decorator(
                                    ctx => _anhuurInterrupter != null && _anhuurInterrupter.IsMe,
                ...
                // jump point handling
                                new Decorator(
                                    ...
                                    new PrioritySelector(
                                        new Decorator(
                                            ctx =>
                                            StyxWoW.Me.Location.DistanceSqr(
                                                _anhuurBeaconSide == AnhuurBeaconSide.Right
                                                    ? rightTopJumpLoc
                                                    : leftTopJumpLoc) > 7 * 7,
                                            new Action(
                ...
                // flip the switches.
                                new Decorator(
                                    ctx =>
                                    _beaconOfLight.DistanceSqr > 3 * 3 &&
                                    (StyxWoW.Me.IsDps() || Targeting.Instance.FirstUnit == null),
                                    new Action(ctx => Navigator.MoveTo(_beaconOfLight.Location))),
                                new Decorator(
                                    ctx => (!hasSnakeAggro || Targeting.Instance.FirstUnit == null) && !StyxWoW.Me.IsHealer(),
                                    ScriptHelpers.CreateInteractWithObject(() => _beaconOfLight, 0, true))
                                )),
                        new Decorator(
                            ctx => !_anhuurIsImmune,
                            new PrioritySelector(
                // interrupt the hymn spell.
                                new Decorator(
                                    ctx => _anhuurInterrupter == StyxWoW.Me,
                                    ScriptHelpers.CreateInterruptCast(() => boss, hymnSpellIds)),
        ...
        [EncounterHandler(48143, "Temple Fireshaper")]
        public Composite TempleFireshaperEncounter()
        {
            WoWUnit fireshaper = null;

            return new PrioritySelector(
                ctx => fireshaper = ctx as WoWUnit,
               ScriptHelpers.CreateDispellEnemy("Molten Barrier", ScriptHelpers.EnemyDispellType.Magic, () => fireshaper));
        }
        ...
        {
            // ToDo : Check why dps sometimes just stand idle
        ...
        public Composite RajhEncounter()
        {
            ...
            return new PrioritySelector(
                ctx => boss = ctx as WoWUnit,
                ScriptHelpers.CreateRunAwayFromBad(ctx => true, 5, SolorFire),
                ScriptHelpers.CreateRunAwayFromBad(ctx => true, 8, SolarWinds));
        }
        ...
        private WoWPlayer GetInterrupter()
        {
            var interrupters = (from partyInfo in DungeonBot.PartyMemberInfos
                                let player = partyInfo.ToPlayer()
                                where
                                    player != null && partyInfo.HasRole(WoWPartyMember.GroupRole.Damage) &&
                                    player.Class != WoWClass.Hunter && player.Class != WoWClass.Warlock
                                select player).ToList();

            if (StyxWoW.Me.IsDps() && StyxWoW.Me.Class != WoWClass.Hunter && StyxWoW.Me.Class != WoWClass.Warlock)
                interrupters.Add(StyxWoW.Me);
            return interrupters.OrderBy(u => u.MaxHealth).FirstOrDefault();
        }
