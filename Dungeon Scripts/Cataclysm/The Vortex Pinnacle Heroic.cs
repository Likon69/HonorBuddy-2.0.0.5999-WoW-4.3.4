...
#endif
{
    public class TheVortexPinnacleHeroic : Dungeon
    {
        #region Overrides of Dungeon

        public override uint DungeonId { get { return 319; } }
        ...
        private const uint AirCurrent = 47305;
        private const uint Twister = 47342;
            ...
            return new PrioritySelector(
                ...
                ScriptHelpers.CreateRunAwayFromBad(
                    ...
                    ObjectManager.GetObjectsOfType<WoWUnit>().Any(
                        u => u.Entry == vortexId && u.Location.Distance2DSqr(_ertan.Location) <= 10*10),
                    8,
                    u => u == _ertan),
                ScriptHelpers.CreateRunAwayFromBad(ctx => true, 6, vortexId),
                new Decorator(
        ...
        [EncounterHandler(43873, "Altairus")]
        public Composite AltairusEncounter()
        {
            return new PrioritySelector(
                ScriptHelpers.CreateRunAwayFromBad(ctx => true,6,Twister));
        }
        ...
        public Composite HowlingGaleEncounter()
        {
            ...
            return new PrioritySelector(
                ...
                orb =
                ObjectManager.GetObjectsOfType<WoWUnit>().Where(
                    u => u.Entry == 45572 && !HowlingGaleBlacklist.Contains(u) && u.DistanceSqr <= 30*30).OrderBy(
        ...
        public Composite YoungStormDragonEncounter()
        {
            ...
            return new PrioritySelector(
                ctx =>
                    {
                        healingWell =
                            ObjectManager.GetObjectsOfType<WoWDynamicObject>().FirstOrDefault(d => d.Entry == 88201);
                        return drake = ctx as WoWUnit;
                    },
                ScriptHelpers.CreateRunAwayFromBad(
                    ctx =>
                    StyxWoW.Me.IsTank() && healingWell != null &&
                    healingWell.Location.Distance2DSqr(drake.Location) <= 20*20,
                    () => healingWell.Location,
                    40,
                    30,
                    u => u == healingWell),
        ...
        public override MoveResult MoveTo(WoWPoint location)
        {
            ...
            if (destinationArea == myArea)
            {
                ...
                {
                    if (tank.Location.Distance2DSqr(_firstAreaSlipStreamLoc1) <= 20*20 ||
                        tank.Location.Distance2DSqr(_firstAreaSlipStreamLoc2) <= 20*20)
                        destinationArea = _secondArea;

                    if (tank.Location.Distance2DSqr(_firstAreaSlipStreamLoc3) <= 20*20 ||
                        tank.Location.Distance2DSqr(_secondAreaSlipStreamLoc1) <= 20*20)
                        destinationArea = _thirdArea;

                    if (tank.HasAura("Slipstream"))
                    {
                        if (tank.Location.Distance2DSqr(_firstAreaSlipStreamLoc1) <= 60*60 ||
                            tank.Location.Distance2DSqr(_firstAreaSlipStreamLoc2) <= 60*60)
                            destinationArea = _secondArea;

                        if (tank.Location.Distance2DSqr(_firstAreaSlipStreamLoc3) <= 60*60 ||
                            tank.Location.Distance2DSqr(_secondAreaSlipStreamLoc1) <= 60*60)
                            destinationArea = _thirdArea;
                    }
                }
            }
